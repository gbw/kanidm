...
pub fn route_setup(state: ServerState) -> Router<ServerState> {
    ...
    #[cfg(feature = "dev-oauth2-device-flow")]
    {
        ...
        // Note: OAUTH2_DEVICE_LOGIN (/ui/oauth2/device) POST is handled by the views router
        // in server/core/src/https/views/mod.rs to avoid duplicate route registration
    }
    // ⚠️  ⚠️   WARNING  ⚠️  ⚠️
    // IF YOU CHANGE THESE VALUES YOU MUST UPDATE OIDC DISCOVERY URLS
    router = router
